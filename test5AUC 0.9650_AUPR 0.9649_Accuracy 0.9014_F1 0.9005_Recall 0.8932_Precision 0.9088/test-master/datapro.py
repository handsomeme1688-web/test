...
def _load_optional_view(base_path, candidates):
    for filename in candidates:
        path = os.path.join(base_path, filename)
        if os.path.exists(path):
            return torch.from_numpy(np.loadtxt(path, dtype=np.float32, delimiter=','))
    return None
...
def Simdata_processing(param):
    ...
    optional_views = {
        'mm_tg': ['m_tg.csv', 'm_target.csv', 'mirna_target.csv', 'mirna_target_similarity.csv'],
        'mm_pw': ['m_pw.csv', 'm_pathway.csv', 'mirna_pathway.csv', 'mirna_pathway_similarity.csv'],
        'dd_onto': ['d_onto.csv', 'd_ontology.csv', 'disease_ontology.csv', 'disease_semantic_ontology.csv'],
    }
    for key, candidates in optional_views.items():
        optional_tensor = _load_optional_view(param.datapath, candidates)
        if optional_tensor is not None:
            dataset[key] = optional_tensor
    return dataset
...
class EdgeDataset(Dataset.Dataset):
    def __init__(self, edges, labels, conf_matrix=None, use_label_confidence=True):
        self.Data = edges
        self.Label = labels
        self.conf_matrix = conf_matrix
        self.use_label_confidence = use_label_confidence
    ...
    def __getitem__(self, index):
        ...
        if self.use_label_confidence and label > 0.5:
